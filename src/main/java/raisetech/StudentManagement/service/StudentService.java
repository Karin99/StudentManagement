...
public class StudentService {
     ...
     * @return 受講生詳細一覧（全件）
     */
    public List<StudentDetail> searchStudentList() throws CustomNotFoundException {
        List<Student> studentList = repository.search();
        List<StudentCourse> studentCourseList = repository.searchStudentCourseList();

        if (studentList.isEmpty()){
            throw new CustomNotFoundException("登録されている受講生はいません。");
        }
     ...
     * @return 受講生詳細情報
     */
    public StudentDetail searchStudent(String id) throws CustomNotFoundException {
        Student student = repository.searchStudent(id);

        if (student == null) {
            throw new CustomNotFoundException("ID " + id + " の学生は登録されていません。");
        }
    ...
    /**
     * 受講生詳細の登録を行います。
     * 受講生情報と受講生コース情報を個別に登録し、受講生コース情報には受講生情報を紐づける値とコース開始日、コース終了日を設定します。
     *
     ...
     */
    @Transactional
    public void updateStudent(StudentDetail studentDetail) throws CustomNotFoundException {
